...
class Command(BaseCommand):
    help = 'Fix doctor IDs that have ADM prefix instead of DOC prefix (Version 3 - handles foreign key constraints)'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('SUSHURSA HEALTHCARE - DOCTOR ID PREFIX FIX V3')
        )
        self.stdout.write(
            self.style.SUCCESS('=' * 60)
        )
        ...
        # Perform the fix using raw SQL with foreign key constraint handling
        self.stdout.write('\n5. Applying changes with foreign key constraint handling...')
        updated_count = 0
        errors = []
        ...
        with connection.cursor() as cursor:
            ...
            self.stdout.write('   → Disabled foreign key constraints')
            ...
            try:
                for i, doctor in enumerate(adm_doctors):
                    try:
                        ...
                        cursor.execute('SELECT COUNT(*) FROM users WHERE id = %s', [new_id])
